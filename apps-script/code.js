...
function doPost(e) {
  ...
  try {
    ...
    if (action === "delete_expense") {
      ...
      if (deleted) {
        return createJsonResponse({ status: "success", message: "ลบธุรกรรมการเงินเรียบร้อยครับพี่" });
      } else {
...
function checkTaskReminders() {
  ...
  for (var i = 1; i < taskRows.length; i++) {
    ...
    if (status !== "Completed" && reminderTime) {
      // ตรวจสอบถ้าเวลาปัจจุบันของพี่ตรงกับช่องตั้งเวลาเตือนภัย
...
function getOrCreateGoogleTaskList() {
  if (typeof Tasks === "undefined") {
    throw new Error("กรุณาเปิดบริการ 'Tasks API' ในเมนู Services ด้านซ้ายของ Google Apps Script ก่อนครับพี่");
  }
...
function checkMonthlyFinancialHealth(ssId) {
  ...
  if (totalIncome === 0) {
    grade = "F";
    icon = "🚨";
    advice = "เตือนภัยระดับสีแดงครับพี่! ในเดือนนี้ยังไม่มีการบันทึกรายรับเข้ามาเลยครับ อย่าลืมบันทึกรายรับเพื่อวิเคราะห์สภาพคล่องที่แท้จริงนะพี่";
  } else if (savingRatio >= 20 && debtRatio <= 30) {
    grade = "A";
    icon = "🏆";
    advice = "ยอดเยี่ยมที่สุดเลยครับพี่! การเงินแข็งแกร่งมาก ออมเยอะหนี้น้อย สุขภาพการเงินฟิตปั๋งระดับเศรษฐีเลย รักษาวินัยนี้ไว้สุดยอดมากครับ!";
  } else if (savingRatio >= 10 && debtRatio <= 40) {
    grade = "B";
    icon = "🌟";
    advice = "ดีมากเลยครับพี่! การเงินอยู่ในระดับแข็งแรง มีเงินออมสม่ำเสมอและหนี้สินไม่เกินขอบเขตปลอดภัย สบายใจได้ครับพี่";
  } else if (savingRatio >= 5 && debtRatio <= 50) {
    grade = "C";
    icon = "⚖️";
    advice = "พอใช้ได้อยู่นะพี่! สภาพคล่องยังหมุนเวียนได้ แต่น้องแนะนำให้พยายามเพิ่มเปอร์เซ็นต์เงินออมขึ้น หรือหาทางลดจ่ายหนี้ลงเพื่อความปลอดภัยนะครับพี่";
  } else if (savingRatio < 5 && debtRatio <= 55) {
    grade = "D";
    icon = "⚠️";
    advice = "น้องว่ามีความเสี่ยงทางการเงินอยู่นะครับพี่! ยอดเงินออมน้อยเกินเกณฑ์มาตรฐานสะสม (ควรมีอย่างน้อย 10%) แนะนำให้พยายามตัดรายจ่ายฟุ่มเฟือยเพื่อเพิ่มสัดส่วนเงินออมด่วนเลยนะพี่";
  } else {
    grade = "F";
    icon = "🚨";
    advice = "สภาพคล่องของพี่มีปัญหารุนแรงแล้วนะพี่! หนี้สินเกิน 50% ของรายได้ และเงินออมต่ำมาก แนะนำให้รีบปรับลดรายจ่าย และหลีกเลี่ยงการก่อหนี้ก้อนใหม่ด่วนที่สุดเลยนะพี่!";
  }
